...
interface DesignV2ArticleDetailsProps {
	className?: string;
	id: string;
}
...
export const DesignV2ArticleDetails: FC<DesignV2ArticleDetailsProps> = memo((props) => {
	...
	const renderBlock = useCallback((block: ArticleBlock) => {
		switch (block.type) {
			case 'text':
				return <ArticleBlockTextComponent key={block.id} block={block} className={cls.ArticleBlockText} />;
			case 'code':
				return <ArticleBlockCodeComponent key={block.id} block={block} className={cls.ArticleBlockCode} />;
			case 'image':
				return <ArticleBlockImageComponent key={block.id} block={block} className={cls.ArticleBlockImage} />;
			default:
				return null;
		}
	}, []);
	...
	if (isLoading) {
		...
		content = <Text align='center' variant='error' title={t('errorLoading')} />;
	} else {
		content = (
			<>
